...
var Room = function(io, name, options) {
  this.io = io;
  this.name = name;
  this.players = [];
...
Room.prototype.playerWon = function(player, s) {
  ...
  this.state = Room.STATE_STOPPED;
  this.emit(Room.EVENT_END, this.playerStats, this.actions);
}
  ...
  if(this.state == Room.STATE_STOPPED) {
    ...
    var countdownTimer = setInterval(function() {
      if(count++ >= 10) {
        ...
        self.broadcast(Message.CHAT, {text: 'Starting in ' + (10 - count + 1), id: null});
      }
    }, 1000);
  }
}
